...
bool Storage::write(uint8_t slot, ByteStream &buffer)
{
  ...
  volatile uint8_t *dest = (volatile uint8_t *)FLASH_STORAGE_SPACE + (MAX_MODE_SIZE * slot);
  // implement device storage here
  for (uint16_t i = 0; i < MAX_MODE_SIZE; ++i) {
...
bool Storage::read(uint8_t slot, ByteStream &buffer)
{
  ...
  volatile uint8_t *src = (volatile uint8_t *)FLASH_STORAGE_SPACE + (MAX_MODE_SIZE * slot);
  // implement device storage here
  for (uint16_t i = 0; i < MAX_MODE_SIZE; ++i) {
    buf[i] = src[i];
  }
