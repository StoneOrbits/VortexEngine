...
Behaviours::Behaviours(VortexEngine &engine) :
  m_engine(engine),
  m_nodes{},
  m_nodeCount(0),
  m_otherMode(engine)
...
bool Behaviours::serialize(ByteStream &stream)
{
  ...
  m_engine.serializeVersion(stream);
...
{
  Mode *nextMode = m_engine.modes().nextMode();
  if (nextMode) {
    // take copy of next mode
    m_otherMode = *nextMode;
  }
  // go back
  m_engine.modes().previousMode();
}
...
{
  switch (type) {
  case NODE_ACCEL_MOTION: return new NodeAccelMotion(m_engine);
  case NODE_ACCEL_NORMALIZED: return new NodeAccelNormalized(m_engine);
  case NODE_ACCEL_CURVED: return new NodeAccelCurved(m_engine);
  case NODE_ACCEL_FILTERED: return new NodeAccelFiltered(m_engine);
  case NODE_ACCEL_DIR_X: return new NodeAccelDirX(m_engine);
  case NODE_ACCEL_DIR_Y: return new NodeAccelDirY(m_engine);
  case NODE_ACCEL_DIR_Z: return new NodeAccelDirZ(m_engine);
  case NODE_ACCEL_PITCH: return new NodeAccelPitch(m_engine);
  case NODE_ACCEL_ROLL: return new NodeAccelRoll(m_engine);
  case NODE_ACCEL_TILT: return new NodeAccelTilt(m_engine);

  case NODE_ABS: return new NodeAbs(m_engine);
  case NODE_ADD: return new NodeAdd(m_engine);
  case NODE_MULTIPLY: return new NodeMultiply(m_engine);
  case NODE_CLAMP: return new NodeClamp(m_engine);
  case NODE_CURVE: return new NodeCurve(m_engine);
  case NODE_THRESHOLD: return new NodeThreshold(m_engine);

  case NODE_MODE_BLEND: return new NodeModeBlend(m_engine);
