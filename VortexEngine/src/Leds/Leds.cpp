...
void Leds::blinkQuadrantOffset(Quadrant target, uint32_t time, uint32_t offMs, uint32_t onMs, RGBColor col)
{
  if ((time % MS_TO_TICKS(offMs + onMs)) < MS_TO_TICKS(onMs)) {
...
void Leds::blinkQuadrant(Quadrant target, uint32_t offMs, uint32_t onMs, RGBColor col)
{
  if ((Time::getCurtime() % MS_TO_TICKS(offMs + onMs)) < MS_TO_TICKS(onMs)) {
    setQuadrant(target, col);
  }
...
void Leds::blinkQuadrantFive(uint32_t offMs, uint32_t onMs, RGBColor col)
{
  if ((Time::getCurtime() % MS_TO_TICKS(offMs + onMs)) < MS_TO_TICKS(onMs)) {
    setQuadrantFive(col);
  }
