...
HueShiftPattern::HueShiftPattern(const PatternArgs &args) :
  ...
  m_flip(0)
{
  m_patternID = PATTERN_HUE_SHIFT;
  REGISTER_ARG(m_shiftSpeed);
...
void HueShiftPattern::onBlinkOn()
{
...
void HueShiftPattern::transitionValue(uint8_t &current, const uint8_t next, bool hue)
{
  // if the values are equal then there's no work to do
  if (next == current) {
    return;
  }
  // otherwise we can just shift as normal in closest direction
  ...
  // shiftspeed is larger than the diff or not.
  diff *= step;
  // Only add the shiftspeed if there is enough difference before the target 
  // otherwise we may overshoot then oscillate around the target
  if (diff > m_shiftSpeed) {
    // a shift speed of 0 will be standard speed
    step += m_shiftSpeed;
  }
